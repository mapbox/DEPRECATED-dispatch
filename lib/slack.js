...
slack.alertToSlack = function(inputMessage, destination, client, slackChannel, callback) {
  ...
  utils.encode({ github: inputMessage.number }, (err, res) => {
    ...
    slack.formatMessage(inputMessage, (err, message, prompt) => {
      ...
      let slackFailureMessage = `Error - dispatch ${inputMessage.requestId} failed to send message to ${destination}, see Slack and Lambda logs for details`;
...
slack.formatMessage = function(inputMessage, callback) {
  try {
    ...
    return callback(err, `dispatch ${inputMessage.requestId} - SNS message parsing error`);
  }
};
